...
Compute the intersections between a set of N points defined by D-dimensional `points` (shape `(D, N)`) and some bounding volumes inside a `bvh`.

The returned [`BVHTraversal`](@ref) `.contacts` field will contain the index pairs
`(iboundingvolume, ipoint)` following the order in `bvh.leaves` and `axes(points, 2)`.
...
traversal = traverse_points(bvh, points, directions)

# Reuse traversal buffers for future contact detection - possibly with different BVHs
traversal = traverse_points(bvh, points, directions, 2, traversal)
...
function traverse_points(
    ...
    @boundscheck begin
        ...
        @argcheck size(points, 1) == ndims(bvh)
    end
    ...
    # No intersections for no points
    if num_points == 0
    ...
    bvtt1, bvtt2, num_bvtt = initial_bvtt(bvh, points, start_level, cache, options)
    num_checks = num_bvtt
    ...
    while level < bvh.tree.levels
        ...
        num_bvtt = traverse_points_nodes!(bvh, points,
    ...
    num_bvtt = traverse_points_leaves!(bvh, points,
                                     bvtt1, bvtt2, num_bvtt,
                                     dst_offsets, options)
...
function initial_bvtt(
    ...
    level_checks = num_real * num_points
    ...
    fill_initial_bvtt_points!(bvtt1, level_nodes, num_real, num_points, options)

    bvtt1, bvtt2, level_checks
end


function fill_initial_bvtt_points!(bvtt1, level_nodes, num_real, num_rays, options)
    backend = get_backend(bvtt1)
    if backend isa GPU
